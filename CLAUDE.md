...
| `models/Final_v1_draft/final1/` | ★ **Source of truth (ชุดทำงานหลัก · 14/06/2026)** — ฉบับ **formal ไทยทางการ** 3 เอกสาร: `spec.full.md` (1/3 แบบ+ข้อกำหนด PIECE 1–5) · `spec.requirement.md` (2/3 รายท่า+demand รับแรง) · `spec.summary.md` (3/3 ผลตรวจ+ฐานราก+ทางแก้ ก/ข/ค+ของสั่งซื้อ) · `policy.liability.md` (เอกสารกำกับ: waiver/แบ่งบทบาท 3 ฝ่าย/กฎหมาย — เป็นร่าง ต้องให้ทนายตรวจ) · **ฐานโหลด = EN 16630 Table 1 (1,942N/จุด รวม dynamic — เลิกใช้ 5kN)** · วัสดุ wall 3.2 ตาม draft (ไม่อิงร้าน) · verdict: **ผ่านหมด** เหลือ 3 จุดเฝ้าระวัง (monkey ต้องมีค้ำยันสามเหลี่ยม · pull-up bar 1.5m margin ปานกลาง · ชิ้นยื่น side-grip/nub) · ⚠️ ทุกไฟล์มี disclaimer + กรอบ consultant-handoff · **กฎแก้บาร์: เพิ่ม wall เท่านั้น ไม่เพิ่ม OD** |
| `models/Final_v1_draft/spec.human.v3.md` | **base spec** (ที่มาของ `final1/spec.full.md`) — ใช้ `final1/` เป็นหลัก · ✅ Bar_Straight=42mm sync · wave path verified |
| `models/Final_v1_draft/research.md` | Research อ้างอิง brand/มาตรฐาน/anchor (ฐานของ v3) |
| `models/Final_v1_draft/spec.human.v2.md` | v2 — reference only (เนื้อหาเดียวกับ v3 · ใช้ v3 เป็นหลัก) |
| `models/Final_v1_draft/spec.human.md` | v1 spec — reference only |
| `docs/spec.md` | Original spec — **superseded**, ignore |
...
│   │   ├── final1/                    ← ★★ SOURCE OF TRUTH (ชุดทำงานหลัก 14/06/2026)
│   │   │   ├── spec.full.md            ← ดีไซน์ละเอียด PIECE 1–5
│   │   │   ├── spec.requirement.md     ← รายท่า + วิเคราะห์รับแรง (demand) PIECE 1–4
│   │   │   └── spec.summary.md         ← ฐานราก + verdict recheck + ทางแก้ a/b/c + ของสั่งซื้อ
│   │   ├── spec.human.v3.md           ← Spec v3 (base ของ final1/ · เดิม source of truth)
│   │   ├── research.md                ← Research: brands / EN 16630 / Hilti anchors
│   │   ├── quotation.predict-price.md ← ★ ราคาประเมินฝั่งเรา (cutting/bending plans จาก v3)
│   │   ├── quotation.full.md          ← ★ ใบยื่นช่าง (เว้นราคาให้จดจริง + คำถาม 3 ข้อ + ลายเซ็น)
│   │   ├── spec.human.v2.md           ← Spec v2 (reference only)
│   │   ├── spec.human.md              ← Spec v1 (reference only)
│   │   └── quotation.md              ← เก่า/stale — ห้ามใช้ (มีชิ้นที่ไม่มีอยู่จริงแล้ว)
...
├── docs/
│   ├── quotation.predict.md           ← ★ Contractor-facing formal quotation template (blank prices)
│   ├── quotation.md                   ← Old placeholder (superseded by Final_v1_draft/quotation.md)
│   ├── measurements.md                ← On-site measured dimensions (fill in)
│   └── spec.md                        ← Original spec (superseded, ignore)
...
> - `models/Final_v1_draft/quotation.predict-price.md` — ราคาประเมิน + แผนตัด/ดัดที่ตรวจ bin-packing แล้ว (2 scenario: ชุบ HDG vs galv+spray)
> - `models/Final_v1_draft/quotation.full.md` — ใบยื่นช่าง เว้นราคาให้จดจริง มีคำถาม 3 ข้อ + เงื่อนไข + ลายเซ็น
> - ⚠️ ไฟล์เก่า `models/Final_v1_draft/quotation.md` และ `docs/quotation.predict.md` = stale (อิง v2) — เก็บไว้อ้างอิงราคาเท่านั้น ห้ามยื่น
> - **ล่าสุด 13/06/2026**: `spec.explore.13062026.md` + `quotation.explore.13062026.md` = delta จากสำรวจร้านจริง (ท่อ OD34→wall 2.0, OD42→wall 2.5, bolt 1/2"×4", เพลท 9mm ทุกแผ่น) — ⚠️ ยังไม่ recompute การรับน้ำหนัก ห้ามถือเป็น final จนกว่าจะผ่านรอบคำนวณ
>
