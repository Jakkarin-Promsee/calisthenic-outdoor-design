...
> 🤖 **ใช้ agent ตัวอื่นที่ไม่ใช่ Claude?** เริ่มที่ [AGENTS.md](AGENTS.md) — คู่มือโปรเจกต์แบบ tool-agnostic. ไฟล์นี้ (CLAUDE.md) คือส่วนเสริมเฉพาะ Claude Code (harness + คลัง Blender code-pattern เต็ม)
...
## Project Status (อัปเดต 26/06/2569)

- ✅ **Release 1 เสร็จ** — repo เปิด public บน GitHub (เจ้าของ [@Jakkarin-Promsee](https://github.com/Jakkarin-Promsee)) · license = [MIT](LICENSE) + disclaimer "functional design ไม่ใช่งานวิศวกรรมรับรอง" (**ห้ามตัด disclaimer ออก**) · README หลัก = อังกฤษ
- 📦 **ส่งมอบแล้ว** — ชุดเอกสาร formal คู่ ไทย (`final1-th/`) + อังกฤษ (`final1-en/`) · โมเดล 3D 5 ชิ้น (`models/final1-blender/`) · เรนเดอร์ 16 ภาพ + walkthrough
- 🔜 **ค้าง / ยังไม่ final (จุดกลับมาทำต่อ)**:
  - (1) `archive/specs/*.explore.13062026.*` = วัสดุจากร้านจริง (wall บางลง) **ยังไม่ recompute การรับน้ำหนัก** — ห้ามถือเป็น final
  - (2) เอกสารทุกฉบับยัง = **ฉบับร่าง** รอวิศวกร กว. (ฝ่าย ค) ตรวจ/รับรองก่อนสร้าง
  - รายการ "งานที่ค้าง" ฉบับเต็ม → [final1-th/tldr.b.md](final1-th/tldr.b.md)
...
├── models/
│   ├── final1-blender/                ← ★ WORKING FOLDER — ใช้ไฟล์ในนี้เป็นหลัก (tracked ใน git)
...
│   ├── archive/                       ← ★ เวอร์ชันเก่าทุกชิ้น (tracked · อ้างอิงเท่านั้น — ห้ามแก้/ลิงก์)
│   │   ├── pullup_bar_v1–v2.blend
│   │   ├── snake_bar_v1–v5.blend
│   │   ├── dip_bar_v1–v3.blend
│   │   ├── monkey_flying_bar_v1–v5.blend  ← v5 ไม่มี Rail_Z0 update → current = v4
│   │   ├── floor_v1.blend             ← duplicate ของ final1-blender/floor_v1
│   │   └── main_scene{,2,3}.blend     ← 3 layout เก่า (stale links — ใช้ final_main_scene แทน)
│   └── _backups/                      ← .blend1 auto-backups (ignore · git-ignored)
├── renders/
...
├── scripts/                           ← inspect_*.py (headless QA) · render_pan/walkthrough.py · make_gif.py
├── LICENSE                            ← MIT + functional-design disclaimer (repo เปิด public)
├── README.md                          ← project overview (English, primary)
├── README-th.md                       ← project overview (Thai)
├── AGENTS.md                          ← tool-agnostic agent guide (สำหรับ agent ตัวอื่น)
└── CLAUDE.md                          ← this file
```
...
>
> **Known issue:** `models/archive/main_scene.blend` (old) has stale link to `dip_bar.blend` (now `dip_bar_v3.blend`).
...
| Editing files in `models/archive/` | Archive = reference only, not the current version | Always open from `models/final1-blender/` |
