# Spec v2 — Calisthenic Equipment

> **อ่านฉบับนี้เป็นหลัก** — spec.human.md คือฉบับเก่า เก็บเป็น reference เท่านั้น
>
...
| Bar_Straight | ท่อกลม | **OD 34mm wall 3.2mm galv** | ตรง |
...
| Bar_Straight | OD 34mm | wall 3.2mm | ยาว **2.000m** (inner face–inner face) | 1 | z=1.966m |
...
> **Wave bar path breakdown**: 38mm (ปักเสา) + 400mm + 8 arcs×157mm + 300mm×4 + 800mm×3 + 400mm + 38mm = **5,730mm**  
...
| Bar_Straight | −1.000 → +1.000m (inner face) | 1.966m | structural, ต่ำกว่า Wave 1×OD=34mm |
| Bar_Wave | −1.038 → +1.038m | 2.000m | wave ±50cm ในแนว Y |
| Side_Grip_L/R | x = ±1.038m | 0.10 → 1.85m | 4 U-grips **แยกชิ้น** ต่อข้าง · ยื่น −Y 25cm |

```
มองจากบน (XY plane) — Wave bar:

  Post_L                                                Post_R
  x=−1.038                                           x=+1.038
   │←38→│←── 400 ──→╮ 300 ╭───── 800 ─────╮ 300 ╭── 400 ──→│←38→│
   │    │             ╰────╯                ╰────╯             │    │
  face       U1(+Y)          U2(−Y)    U3(+Y)     U4(−Y)     face

  4 U-shapes สลับทิศ · กว้าง 50cm · ลึก 50cm · bend R=100mm
```

```
มองจาก X+ — Side grips (per ข้าง, ระนาบ YZ) — 4 ชิ้นแยกอิสระ:
  ...
  z=0.35 ──●──────────────────╮                    ╭── ● weld  Grip 1
  z=0.10 ──●──────────────────╯                    ╰── ● weld
           y=0  y=−0.15  y=−0.25  y=−0.15  y=0
          (เสา)                 (ยื่นสุด)  (เสา)

  · แต่ละ grip = ชิ้นแยก · weld 2 จุด/ชิ้น · H=250mm · ยื่น 250mm · R=100mm ทั้ง 2 มุม
```
...
**Straight bar ต่ำกว่า Wave 1×OD**: ทั้งสองบาร์สัมผัสกันที่ z=1.983m เชื่อมยึดตลอดแนว — Straight รับ structural load หลัก Wave รับแรงจับ
...
| ชิ้น | รูปทรง | สเปก | ขนาด | จำนวน | หมายเหตุ |
|------|--------|------|------|:-----:|---------|
| DipBar_01 | OD 42mm | wall 3.2mm | path **~3.10m** | 1 | x = 0.000m |
| DipBar_02 | OD 42mm | wall 3.2mm | path **~3.10m** | 1 | x = 0.642m |
| DipBar_03 | OD 42mm | wall 3.2mm | path **~3.10m** | 1 | x = 1.184m |
| Nub_01/02/03 | OD 42mm | wall 3.2mm | path **~0.632m** | 3 | frame ละ 1 nub |

> **Frame path breakdown**: 1190mm + arc16mm + 620mm + arc188mm + 1080mm = **3,094mm**  
...
```
Nub profile (มองจาก X axis, ระนาบ YZ):

  z=1.200 ──── 170mm ────╮                   ╰──── 170mm ──── z=1.200
                           ╰── arc R=80 ──40mm── arc R=80 ──╯
                              (−Y → −Z)         (−Z → +Y)
  z=1.000 ────────────────────────────────────────────────── z=1.000
  y=0.000                   y=−0.250                       y=0.000
```
...
ด้านข้าง (มองจาก X+, ระนาบ YZ):
  ...
  y=   −1   0         1           2         3         4
        ↑   ↑         ↑           ↑         ↑         ↑
     entry Post_y0 Post_y1     Post_y2   Post_y3
```
...
```
  z=2.70 ── Tri_Horiz_L/R (แนวนอน, y=4→5) ──●── Z4_04 (rung ปลายสุด)
     ↑                                         │
  Post_y3                               Rail_Z4 │ (hypotenuse เฉียง 35°)
  (แนวตั้ง)                                     │
  y=3 ──────────────────────────────────────── y=4

  มุมฉาก 90° อยู่ที่ยอด Post_y3 (z=2.70m, y=3)
  Z4_04 อยู่ที่จุดปลาย (y=4, z=2.70) — ทำหน้าที่ cross brace แทน Tri_Cap_B
```
